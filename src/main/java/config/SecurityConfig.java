 ...
 * .authorizeRequests() .antMatchers("/api/customers/**").permitAll() // No CSRF
 * protection on these endpoints .anyRequest().authenticated(); // Requires
 * authentication for other requests }
 * 
 * @Bean public UserDetailsService userDetailsService() { User.UserBuilder users
 * = User.withDefaultPasswordEncoder(); return new InMemoryUserDetailsManager(
